...
#    test -x "$(which curl)" || die "You must install curl!"
 ...
 #   curl -s http://ffmpeg.org/releases/ffmpeg-3.1.4.tar.bz2 -o ${ffmpeg_archive} >> ${build_log} 2>&1 || \
  ...
  prefix=${src_root}/ffmpeg/android/arm64
  addi_cflags=""
  addi_ldflags=""
  #export PKG_CONFIG_PATH="${src_root}/openssl-android:${src_root}/rtmpdump/librtmp:${src_root}/openh264"
  ./configure \
    --prefix=${prefix} \
    --enable-shared \
    --disable-static \
